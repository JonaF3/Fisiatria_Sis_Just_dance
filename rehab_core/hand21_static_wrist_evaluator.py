...
class Hand21StaticWristAlignmentEvaluator:
    ...
    def evaluate(self, hand_result=None, frame_index: int = 0, timestamp_s: float = 0.0, **kwargs):
        if hand_result is None:
            ...
            allow_no_hand_release = bool(self.cfg.get("allow_no_hand_release_after_rep", True))

            # Solo despues de una repeticion completada: perder la mano puede ser release.
            if allow_no_hand_release and self.state.completed_reps > 0:
                if self.state.cooldown_frames_left > 0:
                    self.state.cooldown_frames_left -= 1
                    if self.state.cooldown_frames_left <= 0:
                        self.state.phase = "waiting_release"
                    ...
                    return {
                        ...
                        "feedback": "Sal de la postura y vuelve a mostrar la mano extendida.",
                        "hand_state": "no_hand_cooldown",
                        "tracking_quality": 0.0,
                        "open_count": 0,
                        "closed_count": 0,
                        "avg_pip_flexion": None,
                        "avg_pip_flexion_2d": None,
                        "thumb_value": None,
                        "thumb_open": False,
                        "thumb_closed": False,
                        "axis_from_horizontal": None,
                        "wrist_to_middle_tip_from_horizontal": None,
                ...
                if self.require_release_before_count:
                    self.state.phase = "waiting_release"
                    self.state.release_frames += 1
                    if self.state.release_frames >= self.release_frames_required:
                        self.state.armed = True
                        self.state.phase = "waiting_target"
                        feedback = "Release detectado. Ahora vuelve a mostrar la mano extendida en objetivo."
                    else:
                        feedback = f"Release sin mano: {self.state.release_frames}/{self.release_frames_required}."
                    return {
                        "ok": True,
                        "phase": self.state.phase,
                        "rep_completed": False,
                        ...
                        "last_rep": dict(self.state.last_rep),
                        "feedback": feedback,
                        "hand_state": "no_hand_release_after_rep",
                        "tracking_quality": 0.0,
                        "open_count": 0,
                        "closed_count": 0,
                        "avg_pip_flexion": None,
