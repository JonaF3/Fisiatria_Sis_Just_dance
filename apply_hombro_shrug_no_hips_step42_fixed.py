"""
apply_hombro_shrug_no_hips_step42_fixed.py

Fix corregido para ejercicio 'hombro':
- El ejercicio real es subir y bajar hombros.
- No depende de muñeca, índice ni caderas.
- Agrega métrica:
...
Uso:
    python apply_hombro_shrug_no_hips_step42_fixed.py
...
def patch_angles():
    ...
    backup(ANGLES, ".bak_hombro_shrug_no_hips_fixed")
    ...
    # Buscar return angles correctamente.
    ...
    if matches:
        ...
        print("[OK] Agregada métrica shoulder_head_gap_over_width_2d antes de return angles")
        return

    # Fallback: si no encuentra return angles, insertar antes de la sección de métricas posturales
    # o antes del primer def después de compute_angles_33.
    fallback_markers = [
        "\n# -----------------------------------------------------------------------------\n# Métricas posturales",
        "\ndef midpoint_2d",
        "\ndef tracking_quality_for",
    ]

    insert_idx = -1
    for marker in fallback_markers:
        insert_idx = text.find(marker)
        if insert_idx != -1:
            break

    if insert_idx == -1:
        raise RuntimeError(
            "No pude encontrar un punto seguro para insertar la métrica. "
            "Pásame las últimas 80 líneas de rehab_core/angles_33.py."
        )

    text = text[:insert_idx] + "\n" + METRIC_BLOCK + "\n" + text[insert_idx:]
    ANGLES.write_text(text, encoding="utf-8")

    print("[OK] Agregada métrica shoulder_head_gap_over_width_2d usando fallback")
...
def patch_config():
    ...
    backup(CONFIG, ".bak_hombro_shrug_no_hips_fixed")
